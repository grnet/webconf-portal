<?php
/* Library edit by tsipizic to support multi server configuration 2015-08 */
...
namespace App\Libraries\BBB;

//require_once "bbb-config.php";
...
class BigBlueButton {
	 ...
	 */
		...
		*/
		//tsipizic TODO select next available instead of random
		if(!$server_id){
			$bbb = Bbb::orderByRaw("RAND()")->first();
		}
		else{
			$bbb = Bbb::findOrFail($server_id);	
		}
		$this->_securitySalt = $bbb->secret;
		$this->_bbbServerBaseUrl = $bbb->url;
		$this->id = $bbb->id;
	...
	*/
	...
	//tsipizic extra function to check server status
	public function isUp() {
		//check if up
		$ip = gethostbyname(parse_url($this->_bbbServerBaseUrl, PHP_URL_HOST));
		$fp = @fsockopen($ip, 80, $errno, $errstr, 0.4);
		if(!$fp){
			return false;
		}
		//check is correct response
		$resp = @file_get_contents($this->_bbbServerBaseUrl);
	...
	//tsipizic extra function to getUrl
